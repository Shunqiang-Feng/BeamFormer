...
def _crop_csi_tx(csi_matrix, M_from, N_from, M_to, N_to):
    """Crop TX antennas from M_from x N_from to M_to x N_to (top-left corner).
    csi_matrix: [freq, rx, tx] where tx = M_from * N_from
    Returns: [freq, rx, M_to * N_to]
    """
    antenna_grid = torch.zeros(M_from, N_from, dtype=torch.bool)
    antenna_grid[:M_to, :N_to] = True
    mask = antenna_grid.flatten()
    return csi_matrix[:, :, mask]
...
class data_process_4_weight_generator():
    ...
    def generate_query_weights(self, batch_size, indices=None):
    ...
    def generate_query_rss(self, csi_mat, indices=None):
        batch_size = csi_mat.shape[0]
        query_weights = self.generate_query_weights(batch_size, indices=indices)
        query_num = query_weights.shape[1]
        query_weights = query_weights.view(batch_size, query_num, -1)
...
class csi_dataset(Dataset):
    def __init__(self, src_folder, M_tx = 16, N_tx = 16, M_rx = 2, N_rx = 1, add_noise = False, snr_min = None, subcarrier_bw = 240e3, crop = None):
        ...
        # crop: SimpleNamespace with M_tx_original, N_tx_original
        # Load with original (larger) dimensions, then crop to M_tx x N_tx
        if crop is not None:
            self.M_tx_load = crop.M_tx_original
            self.N_tx_load = crop.N_tx_original
            self.crop_M_tx = M_tx
            self.crop_N_tx = N_tx
        else:
            self.M_tx_load = M_tx
    ...
    def __getitem__(self, idx):
        csi_mat = read_csi_file_to_torch(self.dataset_path[idx], M_tx=self.M_tx_load, N_tx=self.N_tx_load, M_rx=self.M_rx, N_rx=self.N_rx)
        if self.crop_M_tx is not None:
            csi_mat = _crop_csi_tx(csi_mat, self.M_tx_load, self.N_tx_load, self.crop_M_tx, self.crop_N_tx)
        if self.add_noise:
...
def load_datasets(c):
    def _make_csi_dataset(path):
        return csi_dataset(
            ...
            crop=getattr(c.dataset, 'crop', None),
        )
