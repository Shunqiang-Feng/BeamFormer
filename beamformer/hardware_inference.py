...
from .utils import load_config, gpu_tensor_to_np, _draw_polar_disk_subplot, _make_disk_grid
...
def compare_scan_with_pred(point_coords, energy_values, pred_tensor,
    ...
    # --- shared grid ---
    x_grid, y_grid, mask, r_max = _make_disk_grid(theta_max)
    ...
    x = np.zeros(N)
    y = np.zeros(N)
    for i, (phi, theta) in enumerate(point_coords):
    ...
    _draw_polar_disk_subplot(
        ax1, x, y, energy_array,
        x_grid, y_grid, mask, r_max, cmap,
        title='Ground Truth',
        show_colorbar=show_colorbar, show_colorbar_label=show_colorbar_label,
        colorbar_label='Energy Intensity',
        scatter_xy=(x, y),
    )
    ...
    r_pred = np.sin(THETA_GRID)
    x_pred = (r_pred * np.cos(PHI_GRID)).ravel()
    y_pred = (r_pred * np.sin(PHI_GRID)).ravel()
    ...
    if show_peak_coordinates:
        ...
        peak_phi_deg = phi_values[max_idx[0]]
        peak_theta_deg = theta_values[max_idx[1]]
        peak_r = np.sin(np.radians(peak_theta_deg))
        peak_xy = (peak_r * np.cos(np.radians(peak_phi_deg)),
                   peak_r * np.sin(np.radians(peak_phi_deg)))
        peak_label = f'Peak: θ={peak_theta_deg:.1f}°, φ={peak_phi_deg:.1f}°'

    _draw_polar_disk_subplot(
        ax2, x_pred, y_pred, pred_normalized.ravel(),
        x_grid, y_grid, mask, r_max, cmap,
        title='Prediction',
        show_colorbar=show_colorbar, show_colorbar_label=show_colorbar_label,
        colorbar_label='Normalized Intensity',
        peak_xy=peak_xy, peak_label=peak_label,
    )
    ...
    interpolated_data = {
        ...
        'energy_grid': np.where(mask, griddata(np.column_stack([x, y]), energy_array,
                                               (x_grid, y_grid), method='linear', fill_value=0.0), np.nan),
        'x_grid': x_grid,
        'y_grid': y_grid,
    }
