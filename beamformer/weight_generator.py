...
def transform_weights_full_array(B):
...
class ParametricGenerator(nn.Module):
    ...
    def generate(self, z=None, hardware = False):
        raw_weights = self.forward(z)
        if not hardware:
            weight, active_antenna = transform_weights(raw_weights)
        else:
            weight, active_antenna = transform_weights_full_array(raw_weights)
...
class PredefinedGenerator():
    ...
    def _generate_small_shape(self):
        ...
        elif self.sample_mode == "single-arm":
            random_indices = torch.randperm(self.angle_steps_theta * self.angle_steps_phi)[:self.sample_num]
            self._last_grid_indices = random_indices  # expose for CNNGeneratorModel
            template_weights = self._generate_query_weights()
            weights = template_weights[:, :, random_indices]
