"""
Figure 15: Comparison of model latencies.
...
def plot(data, save_path):
    ...
    for qn in query_numbers:
        if qn >= 1000:
            ...
            if k_value == int(k_value):
                x_labels.append(f'{int(k_value)}K')
            else:
                formatted_value = f'{k_value:.1f}'.rstrip('0').rstrip('.')
                x_labels.append(f'{formatted_value}K')
        else:
            x_labels.append(str(int(qn)))

    ax.set_xticklabels(x_labels, fontsize=FONT_SIZE_TICK, family='DejaVu Sans')
    ...
    ax.set_xlabel('Query Number', fontsize=FONT_SIZE_LABEL, family='DejaVu Sans')
    ax.set_ylabel('Latency (ms)', fontsize=FONT_SIZE_LABEL, family='DejaVu Sans')
