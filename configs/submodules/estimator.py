...
def CNNGenerator(unet_base_channels=80,
                  estimator_pretrained_model=None,
                  estimator_lr_scheduler_type="cosine"):
...
def MLPAutoEncoder(array_factor_len=1024, dim=4096,
                   angle_steps_phi=80, angle_steps_theta=20,
                   estimator_pretrained_model=None,
                   estimator_lr_scheduler_type="cosine"):
    return SimpleNamespace(
        type="mlp_ae",
        array_factor_len=array_factor_len,
        dim=dim,
        angle_steps_phi=angle_steps_phi,
        angle_steps_theta=angle_steps_theta,
        estimator_pretrained_model=estimator_pretrained_model,
        estimator_lr_scheduler_type=estimator_lr_scheduler_type,
        optimizer="adamw",
    )
